#include "ConservativeAdvectionSchmidSSD_12.h"
#include "SystemBase.h"
#include "libmesh/utility.h"

registerMooseObject("cdf_updateApp", ConservativeAdvectionSchmidSSD_12);

InputParameters
ConservativeAdvectionSchmidSSD_12::validParams()
{
...
ConservativeAdvectionSchmidSSD_12::ConservativeAdvectionSchmidSSD_12(
...
Real
ConservativeAdvectionSchmidSSD_12::negSpeedQp()
{
  Real edge_sign;
...
Real
ConservativeAdvectionSchmidSSD_12::computeQpResidual()
{
  // This is the no-upwinded version
  // It gets called via Kernel::computeResidual()
  return negSpeedQp() * _u[_qp] -
         _edge_dislocation_increment[_qp][_slip_sys_index] * _test[_i][_qp];
}
...
ConservativeAdvectionSchmidSSD_12::computeQpJacobian()
{
  ...
  Real source_jac =
      _d_edge_dislocation_increment_d_rho[_qp][_slip_sys_index] * _phi[_j][_qp] * _test[_i][_qp];
...
void
ConservativeAdvectionSchmidSSD_12::computeResidual()
{
  switch (_upwinding)
  {
...
void
ConservativeAdvectionSchmidSSD_12::computeJacobian()
{
  switch (_upwinding)
  {
    case UpwindingType::none:
...
void
ConservativeAdvectionSchmidSSD_12::fullUpwind(JacRes res_or_jac)
{
  ...
  for (_i = 0; _i < num_nodes; ++_i)
  {
    for (_qp = 0; _qp < _qrule->n_points(); _qp++)
    {
      ...
      _local_re(_i) += -1.0 * source_residual * JxW;
      ...
      if (res_or_jac == JacRes::CALCULATE_JACOBIAN)
      {
        for (_j = 0; _j < _phi.size(); _j++)
        {
          ...
          Real source_jac = _d_edge_dislocation_increment_d_rho[_qp][_slip_sys_index] *
                            _phi[_j][_qp] * _test[_i][_qp];

          _local_ke(_i, _j) += -1.0 * source_jac * JxW;
        }
      }
    }
  }
