...
MatCoupledDisloDensity::validParams()
{
  ...
  params.addRequiredCoupledVar("v", "The coupled variables which provide the dislocation density as source and sink terms");
...
MatCoupledDisloDensity::computeQpResidual()
{
  ...
  case DisloCharacter::edge:
    ...
    sink = - _Ce / _burgers * 2.0 * _de * (temp_type_dislocation_density + _u[_qp]) * abs_slip_increment;
    break;
...
MatCoupledDisloDensity::computeQpJacobian()
{
  ...
  {
  case DisloCharacter::edge:
    source = _Ce * _ke_b * 0.5 / std::sqrt(total_dislocation_density + _u[_qp]) * abs_slip_increment;
    sink = - _Ce / _burgers * 2.0 * _de * abs_slip_increment;
    break;
  case DisloCharacter::screw:
    source = _Cs * _ks_b * 0.5 / std::sqrt(total_dislocation_density + _u[_qp]) * abs_slip_increment;
    sink = - _Cs / _burgers *
