...
registerMooseObject("cdf_updateApp", DisloVelocity_2D4);

InputParameters
DisloVelocity_2D4::validParams()
{
...
DisloVelocity_2D4::DisloVelocity_2D4(const InputParameters & parameters)
...
void
DisloVelocity_2D4::computeQpProperties()
{
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    _dislo_velocity[_qp][i] = _slip_rate[_qp] / _burgersvector / (_rho_edge[_qp] + _scale * _rho_screw[_qp]);
  }
}

void
DisloVelocity_2D4::initQpStatefulProperties()
{
