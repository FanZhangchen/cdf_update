#include "SSDUpdate_sim.h"

registerMooseObject("cdf_updateApp", SSDUpdate_sim);

InputParameters
SSDUpdate_sim::validParams()
{
  ...
  params.addCoupledVar("dislo_den_pos_1", 0.0, "Positive edge dislocation");
  params.addCoupledVar("dislo_den_neg_1", 0.0, "Negative edge dislocation");
  ...
  params.addCoupledVar("dislo_den_pos_2", 0.0, "Positive edge dislocation");
  params.addCoupledVar("dislo_den_neg_2", 0.0, "Negative edge dislocation");
...
SSDUpdate_sim::SSDUpdate_sim(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _dislo_source_edge(getParam<Real>("dislo_source_edge")),
...
void
SSDUpdate_sim::initQpStatefulProperties()
{

  std::vector<Real> edge_dislo_den_pos(_nss);
  std::vector<Real> edge_dislo_den_neg(_nss);
  ...
  _d_edge_dislocation_increment_d_rho[_qp].resize(_nss);

  Real TotalDislocationDensity_ALL;

  edge_dislo_den_pos[0] = _edge_dislo_den_pos_1[_qp];
  edge_dislo_den_pos[1] = _edge_dislo_den_pos_2[_qp];
  ...
  edge_dislo_den_neg[0] = _edge_dislo_den_neg_1[_qp];
  edge_dislo_den_neg[1] = _edge_dislo_den_neg_2[_qp];
...
void
SSDUpdate_sim::computeQpProperties()
{
  std::vector<Real> edge_dislo_den_pos(_nss);
  std::vector<Real> edge_dislo_den_neg(_nss);
  // std::vector<Real> edge_dislo_den_Q3(_nss);
